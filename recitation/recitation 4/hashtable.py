...
from typing import Any, Hashable, Callable, List, Union
...
class HashTable:
    ...
    num_collisions: the total number of collisions
    """
    table: List[Union[Entry, None]]
    size: int
    capacity: int
...
def create_hash_table(hash_function: Callable[[Hashable], int], capacity: int) -> HashTable:
    """
    ...
    for i in range(capacity):
        table.append(None)
    hash_table = HashTable(table, 0, capacity, hash_function, 0)
    return hash_table
...
def hash_table_to_str(hash_table: HashTable) -> str:
    ...
    result += "Collisions: " + str(hash_table.num_collisions)
    return result
...
def put(hash_table: HashTable, key: Hashable, value: Any) -> Any:
    ...
    while hash_table.table[index] is not None and hash_table.table[index].key != key:
        index = (index + 1) % hash_table.capacity
        if index == start_index:
            raise Exception("Hash table is full.")  # make sure not to go in circles
...
def get(hash_table: HashTable, key: Hashable) -> Any:
    ...
    while hash_table.table[index] is not None and hash_table.table[index].key != key:
        index = (index + 1) % hash_table.capacity
        if index == start_index:
            raise Exception("Hash table does not contain key.")  # make sure not to go in circles
