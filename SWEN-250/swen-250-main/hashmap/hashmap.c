...
struct binEntry *lookup(char *key)
{
    ...
    //at this point entry is pointing to the thing we're looking for
	return entry;
}
...
void drop(char *key)
{
    ...
    struct binEntry *entry_to_drop = lookup(key);

    if(entry_to_drop == NULL){
        return;
    }

    if(entry_to_drop->prev == NULL){
        if(entry_to_drop->next != NULL){
            entry_to_drop->next->prev = NULL;
            //case where the entry to drop is at the start
        }
    } else if(entry_to_drop->next == NULL){
        entry_to_drop->prev->next = NULL;
        //case where the entry to drop is at the end
    } else{
        entry_to_drop->prev->next = entry_to_drop->next;
        entry_to_drop->next->prev = entry_to_drop->prev;
        //everything else
    }
    hashtable[hash(key)] = entry_to_drop->next;
    free(entry_to_drop);
}
