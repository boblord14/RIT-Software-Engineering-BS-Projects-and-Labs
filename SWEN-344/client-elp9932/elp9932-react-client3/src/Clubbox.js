...
class Clubbox extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            clubData: this.props.club.count,
            minusButton: true,
            plusButton: false,
            editModal: false
        };
    ...
    yellowChanged(event) {
        ...
        if(change.trim()===""){change=0;}
        let isnum = /^\d+$/.test(change);
        if(isnum){
            var temp = new club(this.state.club.name, this.state.club.count, change, this.state.club.max, this.state.club.city, this.state.club.music, this.state.club.id);
            this.setState({club: temp});  
        }
    ...
    maxChanged(event) {
        ...
        let isnum = /^\d+$/.test(change);
        if(isnum){
            var temp = new club(this.state.club.name, this.state.club.count, this.state.club.yellow,change, this.state.club.city, this.state.club.music, this.state.club.id);
            this.setState({club: temp});
        }
    }
    ...
    updateCount(direction) {
        if ((direction === '+') && (this.state.club.count < this.state.club.max)) {
            var temp = new club(this.state.club.name, this.state.club.count+1, this.state.club.yellow, this.state.club.max, this.state.club.city, this.state.club.music, this.state.club.id);
            this.setState({club: temp});
            if (this.state.club.count === this.state.club.max - 1) {
                this.setState({ plusButton: true });
            } else {
                this.setState({ plusButton: false });
            }
            this.setState({ minusButton: false });
        } else if ((direction === '-') && (this.state.club.count > 0)) {
            var temp = new club(this.state.club.name, this.state.club.count-1, this.state.club.yellow, this.state.club.max, this.state.club.city, this.state.club.music, this.state.club.id);
            this.setState({club: temp});
            if (this.state.club.count === 1) {
        ...
        return (
            <Card className="clubBox" id="box" style={{ backgroundColor: color }}>
                ...
                <CardBody>
                    ...
                    <button className="upButton" type="button" onClick={() => { this.updateCount('+') }} id="addbutton" disabled={this.state.plusButton}>+</button>
                    <button className="downButton" type="button" onClick={() => { this.updateCount('-') }} id="removebutton" disabled={this.state.minusButton}>-</button>
                </CardBody>
                <CardFooter>
                ...
                <Modal isOpen={this.state.editModal}>
                    ...
                    <ModalFooter>
                        <button onClick={() => { this.openModal() }} type="button">Close</button>
                    </ModalFooter>
                </Modal>
            </Card>
        );
