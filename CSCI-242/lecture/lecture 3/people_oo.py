...
class Person:
    ...
    ip: str
    # !! Additional method
    def __lt__(self: "Person", other: "Person"):
...
def binary_search_rec( people: list[ Person ], person: Person,
                       start: int, end: int ) -> int:
    """
    The private recursive helper function for performing a binary search
    for a Person.
    :param people: A list of Person (potentially unordered)
    :param person: the complete name of the person being sought
    ...
    if person.first == mid_value.first and person.last == mid_value.last:
        return mid_index
    # if person is to left of middle, search there
    elif person < mid_value:
        return binary_search_rec( people, person, start, mid_index - 1 )
    # person is right of middle, search there
    else:
        return binary_search_rec( people, person, mid_index + 1, end )
...
def binary_search( people: list[ Person ], first: str, last: str ) -> int:
    ...
    return binary_search_rec(
        people,
        Person( None, first, last, None, None, None),
        0, len( people ) - 1
    )
