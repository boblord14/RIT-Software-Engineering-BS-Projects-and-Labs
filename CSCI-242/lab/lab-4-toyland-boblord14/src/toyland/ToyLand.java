...
public class ToyLand {
    ...
    public void playTime() {
        while (activeToys.size() != 0) {
            IToy currentToy = activeToys.get(0);
            activeToys.remove(0);
            currentToy.play(getRandomTime());
            if (currentToy.isRetired()) {
                retiredToys.add(currentToy);
            } else {
                activeToys.add(currentToy);
            }
        }
    }
    ...
    public void displayStatistics() {
        ...
        for (IToy retiredToy : retiredToys) {
            System.out.println("\t" + retiredToy.toString());
            totalHappiness += retiredToy.getHappiness();
            totalWear += retiredToy.getWear();
        }
        System.out.println("Total toys: " + retiredToys.size());
        System.out.println("Total happiness: " + totalHappiness);
        System.out.println("Average wear: " + (totalWear/retiredToys.size()));
    }
